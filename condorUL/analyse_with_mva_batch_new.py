...
def MET_pt(fname, tree, type):
    if (year == "2017"):
      if ('SingleMuon' in str(fname) or 'SingleElectron' in str(fname) or 'EGamma' in str(fname)):
        return tree.METFixEE2017_pt
      else:
        Met_jesUp_4vec, Met_jesDown_4vec, Met_jerUp_4vec, Met_jerDown_4vec = ROOT.TLorentzVector(), ROOT.TLorentzVector(), ROOT.TLorentzVector(), ROOT.TLorentzVector()
        Met_4vec = ROOT.TLorentzVector()
        Met_4vec.SetPtEtaPhiM(tree.MET_pt_nom, 0, tree.MET_phi_nom, 0)
        MetEE_4vec = ROOT.TLorentzVector()
        MetEE_4vec.SetPtEtaPhiM(tree.METFixEE2017_pt, 0, tree.METFixEE2017_phi, 0)
        if ('nominal' in str(type)):
          return tree.METFixEE2017_pt
        elif ('jesUp' in str(type)):
          Met_jesUp_4vec.SetPtEtaPhiM(tree.MET_pt_jesTotalUp, 0, tree.MET_phi_jesTotalUp, 0)
          return (MetEE_4vec + Met_jesUp_4vec - Met_4vec).Pt()
        elif ('jesDown' in str(type)):
          Met_jesDown_4vec.SetPtEtaPhiM(tree.MET_pt_jesTotalDown, 0, tree.MET_phi_jesTotalDown, 0)
          return (MetEE_4vec + Met_jesDown_4vec - Met_4vec).Pt()
        elif ('jerUp' in str(type)):
          Met_jerUp_4vec.SetPtEtaPhiM(tree.MET_pt_jerUp, 0, tree.MET_phi_jerUp, 0)
          return (MetEE_4vec + Met_jerUp_4vec - Met_4vec).Pt()
        elif ('jerDown' in str(type)):
          Met_jerDown_4vec.SetPtEtaPhiM(tree.MET_pt_jerDown, 0, tree.MET_phi_jerDown, 0)
          return (MetEE_4vec + Met_jerDown_4vec - Met_4vec).Pt()
    ...
    if (year == "2017"):
      if ('SingleMuon' in str(fname) or 'SingleElectron' in str(fname) or 'EGamma' in str(fname)):
        ...
        if ('nominal' in str(type)):
          ...
          Met_jesUp_4vec.SetPtEtaPhiM(tree.MET_pt_jesTotalUp, 0, tree.MET_phi_jesTotalUp, 0)
          return (MetEE_4vec + Met_jesUp_4vec - Met_4vec).Phi()
        elif ('jesDown' in str(type)):
          Met_jesDown_4vec.SetPtEtaPhiM(tree.MET_pt_jesTotalDown, 0, tree.MET_phi_jesTotalDown, 0)
          return (MetEE_4vec + Met_jesDown_4vec - Met_4vec).Phi()
        elif ('jerUp' in str(type)):
          Met_jerUp_4vec.SetPtEtaPhiM(tree.MET_pt_jerUp, 0, tree.MET_phi_jerUp, 0)
          return (MetEE_4vec + Met_jerUp_4vec - Met_4vec).Phi()
        elif ('jerDown' in str(type)):
          Met_jerDown_4vec.SetPtEtaPhiM(tree.MET_pt_jerDown, 0, tree.MET_phi_jerDown, 0)
          return (MetEE_4vec + Met_jerDown_4vec - Met_4vec).Phi()
    elif (year == "2018" or year == "2016"): 
      if ('SingleMuon' in str(fname) or 'SingleElectron' in str(fname) or 'EGamma' in str(fname)):
        return tree.MET_phi
      else:
        if ('nominal' in str(type)):    return tree.MET_phi_nom #jer for smeared 
...
def Jet_flavour(entry, idx):
    ...
    return flavour
#guru
eff_file = ROOT.TFile.Open("bTagEff_"+year+".root")
eff_hist_b = eff_file.Get("eff_b")
eff_hist_c = eff_file.Get("eff_c")
eff_hist_l = eff_file.Get("eff_l")
eff_hist_b_Tight = eff_file.Get("eff_b_Tight")
eff_hist_c_Tight = eff_file.Get("eff_c_Tight")
eff_hist_l_Tight = eff_file.Get("eff_l_Tight")
def Jet_bTagEff(entry, idx, jetpt, jeteta, cut):
  eff = 0
  #eff_file = ROOT.TFile.Open("root://cmsxrootd.fnal.gov//store/user/greddy/condorAtanu/bTagEff_"+year+".root")
  if (abs (entry.Jet_partonFlavour[idx]) ==  5): 
     if(cut == "_Tight"): eff = eff_hist_b_Tight.GetBinContent(eff_hist_b_Tight.GetXaxis().FindBin(jetpt), eff_hist_b_Tight.GetYaxis().FindBin(jeteta))
     else: eff = eff_hist_b.GetBinContent(eff_hist_b.GetXaxis().FindBin(jetpt), eff_hist_b.GetYaxis().FindBin(jeteta))
  elif (abs (entry.Jet_partonFlavour[idx]) ==  4):
     if(cut == "_Tight"): eff = eff_hist_c_Tight.GetBinContent(eff_hist_c_Tight.GetXaxis().FindBin(jetpt), eff_hist_c_Tight.GetYaxis().FindBin(jeteta))
     else: eff = eff_hist_c.GetBinContent(eff_hist_c.GetXaxis().FindBin(jetpt), eff_hist_c.GetYaxis().FindBin(jeteta))
  else : 
     if(cut == "_Tight"): eff = eff_hist_l_Tight.GetBinContent(eff_hist_l_Tight.GetXaxis().FindBin(jetpt), eff_hist_l_Tight.GetYaxis().FindBin(jeteta))
     else: eff = eff_hist_l.GetBinContent(eff_hist_l.GetXaxis().FindBin(jetpt), eff_hist_l.GetYaxis().FindBin(jeteta))
  return eff
...
#guru
sf_file = ROOT.TFile.Open("ElectronTrigSF"+year+".root")
def Ele_trig_SF(lepeta, leppt, err):
  sf = 1
  cut = ""
  if (leppt <= 200):
    cut = "Low"
    ...
    cut = "High"
  sf_hist = sf_file.Get("ScaleFactors"+cut)
  x=sf_hist.GetX()
  y=sf_hist.GetY()
  for i in range(sf_hist.GetN()):#graph has 10 points
    if (lepeta >= x[i]-sf_hist.GetErrorXlow(i) and lepeta < x[i]+sf_hist.GetErrorXhigh(i)):
      if(err == "Up"):
        sf = y[i]+sf_hist.GetErrorYhigh(i)
      elif(err == "Down"):
        sf = y[i]-sf_hist.GetErrorYlow(i)
      else:# ""
        sf = y[i]
    else: sf = 1
  return sf

def quadradd(sys,stat,nom):
  err = np.sqrt(np.square(nom-sys)+np.square(nom-stat))
  return abs(err)
...
def MET_Filters(file, tree):
    if (year == "2016"):
      ...
      if (isMC(file)):
        if (tree.Flag_goodVertices > 0 and tree.Flag_globalSuperTightHalo2016Filter > 0 and tree.Flag_HBHENoiseFilter > 0 and  tree.Flag_HBHENoiseIsoFilter > 0 and tree.Flag_EcalDeadCellTriggerPrimitiveFilter > 0 and tree.Flag_BadPFMuonFilter > 0 and tree.Flag_ecalBadCalibFilter > 0): 
          return 1
        else:
          return 0
      else:
        if (tree.Flag_goodVertices > 0 and tree.Flag_globalSuperTightHalo2016Filter > 0 and tree.Flag_HBHENoiseFilter > 0 and  tree.Flag_HBHENoiseIsoFilter > 0 and tree.Flag_EcalDeadCellTriggerPrimitiveFilter > 0 and tree.Flag_BadPFMuonFilter > 0 and tree.Flag_eeBadScFilter > 0 and tree.Flag_ecalBadCalibFilter > 0): 
          return 1
        else:
          return 0
    else: return 0
...
def fillSkim(skimName):
    ...
    #guru
    sktree.Branch("EvtWt_Ele_IDISO_Sf", EvtWt_Ele_IDISO_Sf, 'EvtWt_Ele_IDISO_Sf/F')
    sktree.Branch("EvtWt_Ele_RECO_Sf", EvtWt_Ele_RECO_Sf, 'EvtWt_Ele_RECO_Sf/F')
    sktree.Branch("EvtWt_Ele_Trig_Sf", EvtWt_Ele_Trig_Sf, 'EvtWt_Ele_Trig_Sf/F')
    sktree.Branch("EvtWt_Mu_ISO_Sf", EvtWt_Mu_ISO_Sf, 'EvtWt_Mu_ISO_Sf/F')
    sktree.Branch("EvtWt_Mu_ID_Sf", EvtWt_Mu_ID_Sf, 'EvtWt_Mu_ID_Sf/F')
    sktree.Branch("EvtWt_Mu_Trig_Sf", EvtWt_Mu_Trig_Sf, 'EvtWt_Mu_Trig_Sf/F')
    sktree.Branch("EvtWt_Ele_IDISO_Sf_Up", EvtWt_Ele_IDISO_Sf_Up, 'EvtWt_Ele_IDISO_Sf_Up/F')
    sktree.Branch("EvtWt_Ele_IDISO_Sf_Down", EvtWt_Ele_IDISO_Sf_Down, 'EvtWt_Ele_IDISO_Sf_Down/F')  
    sktree.Branch("EvtWt_Ele_RECO_Sf_Up", EvtWt_Ele_RECO_Sf_Up, 'EvtWt_Ele_RECO_Sf_Up/F')
    sktree.Branch("EvtWt_Ele_RECO_Sf_Down", EvtWt_Ele_RECO_Sf_Down, 'EvtWt_Ele_RECO_Sf_Down/F')
    sktree.Branch("EvtWt_Ele_Trig_Sf_Up", EvtWt_Ele_Trig_Sf_Up, 'EvtWt_Ele_Trig_Sf_Up/F')
    sktree.Branch("EvtWt_Ele_Trig_Sf_Down", EvtWt_Ele_Trig_Sf_Down, 'EvtWt_Ele_Trig_Sf_Down/F')
    sktree.Branch("EvtWt_Mu_ISO_Sf_Up", EvtWt_Mu_ISO_Sf_Up, 'EvtWt_Mu_ISO_Sf_Up/F')
    sktree.Branch("EvtWt_Mu_ISO_Sf_Down", EvtWt_Mu_ISO_Sf_Down, 'EvtWt_Mu_ISO_Sf_Down/F')
    sktree.Branch("EvtWt_Mu_ID_Sf_Up", EvtWt_Mu_ID_Sf_Up, 'EvtWt_Mu_ID_Sf_Up/F')
    sktree.Branch("EvtWt_Mu_ID_Sf_Down", EvtWt_Mu_ID_Sf_Down, 'EvtWt_Mu_ID_Sf_Down/F')
    sktree.Branch("EvtWt_Mu_Trig_Sf_Up", EvtWt_Mu_Trig_Sf_Up, 'EvtWt_Mu_Trig_Sf_Up/F')
    sktree.Branch("EvtWt_Mu_Trig_Sf_Down", EvtWt_Mu_Trig_Sf_Down, 'EvtWt_Mu_Trig_Sf_Down/F')
...
evtwt_Mu_ISO_Sf_Down):
    ...
    #guru
    EvtWt_Ele_IDISO_Sf[0] = evtwt_Ele_IDISO_Sf
    EvtWt_Ele_RECO_Sf[0] = evtwt_Ele_RECO_Sf
    EvtWt_Ele_Trig_Sf[0] = evtwt_Ele_Trig_Sf
    EvtWt_Mu_ISO_Sf[0] = evtwt_Mu_ISO_Sf
    EvtWt_Mu_ID_Sf[0] = evtwt_Mu_ID_Sf
    EvtWt_Mu_Trig_Sf[0] = evtwt_Mu_Trig_Sf
    EvtWt_Ele_IDISO_Sf_Up[0] = evtwt_Ele_IDISO_Sf_Up
    EvtWt_Ele_IDISO_Sf_Down[0] = evtwt_Ele_IDISO_Sf_Down
    EvtWt_Ele_RECO_Sf_Up[0] = evtwt_Ele_RECO_Sf_Up
    EvtWt_Ele_RECO_Sf_Down[0] = evtwt_Ele_RECO_Sf_Down
    EvtWt_Ele_Trig_Sf_Up[0] = evtwt_Ele_Trig_Sf_Up
    EvtWt_Ele_Trig_Sf_Down[0] = evtwt_Ele_Trig_Sf_Down
    EvtWt_Mu_ISO_Sf_Up[0] = evtwt_Mu_ISO_Sf_Up
    EvtWt_Mu_ISO_Sf_Down[0] = evtwt_Mu_ISO_Sf_Down
    EvtWt_Mu_ID_Sf_Up[0] = evtwt_Mu_ID_Sf_Up
    EvtWt_Mu_ID_Sf_Down[0] = evtwt_Mu_ID_Sf_Down
    EvtWt_Mu_Trig_Sf_Up[0] = evtwt_Mu_Trig_Sf_Up
    EvtWt_Mu_Trig_Sf_Down[0] = evtwt_Mu_Trig_Sf_Down
...
for i in  range(1):
  ...
  #guru
  EvtWt_Ele_IDISO_Sf = array.array('f', [0])
  EvtWt_Ele_RECO_Sf = array.array('f', [0])
  EvtWt_Ele_Trig_Sf = array.array('f', [0])
  EvtWt_Mu_ISO_Sf = array.array('f', [0])
  EvtWt_Mu_ID_Sf = array.array('f', [0])
  EvtWt_Mu_Trig_Sf = array.array('f', [0])
  EvtWt_Ele_IDISO_Sf_Up = array.array('f', [0])
  EvtWt_Ele_IDISO_Sf_Down = array.array('f', [0])
  EvtWt_Ele_RECO_Sf_Up = array.array('f', [0])
  EvtWt_Ele_RECO_Sf_Down = array.array('f', [0])
  EvtWt_Ele_Trig_Sf_Up = array.array('f', [0])
  EvtWt_Ele_Trig_Sf_Down = array.array('f', [0])
  EvtWt_Mu_ISO_Sf_Up = array.array('f', [0])
  EvtWt_Mu_ISO_Sf_Down = array.array('f', [0])
  EvtWt_Mu_ID_Sf_Up = array.array('f', [0])
  EvtWt_Mu_ID_Sf_Down = array.array('f', [0])
  EvtWt_Mu_Trig_Sf_Up = array.array('f', [0])
  EvtWt_Mu_Trig_Sf_Down = array.array('f', [0])
  ...
  #AI: Add Tight b-tag efficiency map
  effmap_pt_eta_b_btagged_Tight = ROOT.TH2D("effmap_pt_eta_b_btagged_Tight", "", 100, 30, 1030, 10, -3, 3)
  effmap_pt_eta_c_btagged_Tight = ROOT.TH2D("effmap_pt_eta_c_btagged_Tight", "", 100, 30, 1030, 10, -3, 3)
  effmap_pt_eta_l_btagged_Tight = ROOT.TH2D("effmap_pt_eta_l_btagged_Tight", "", 100, 30, 1030, 10, -3, 3)
  ...
  systematicS = ["nominal"]
  if (doSys):
    systematicS = ["nominal", "PileupUp", "PileupDown", "BTaghfSFUp", "BTaghfSFDown", "BTaglfSFUp", "BTaglfSFDown", "topptweightUp", "topptweightDown", "jerUp", "jerDown", "jesUp", "jesDown", "LHEScale1", "LHEScale2", "LHEScale3", "LHEScale4", "LHEScale6", "LHEScale8", "LHEScaleUpWeight", "LHEScaleDownWeight", "LHEPdfUpWeight", "LHEPdfDownWeight","TTJets_ST_ScalingUp", "TTJets_ST_ScalingDown", "NoPileup", "L1PreFiringWeight_Up","L1PreFiringWeight_Dn", "Electron_IDISO_Sf_Up", "Electron_IDISO_Sf_Down", "Electron_RECO_Sf_Up","Electron_RECO_Sf_Down","Electron_Trig_Sf_Up","Electron_Trig_Sf_Down","Muon_Trig_Sf_Up","Muon_Trig_Sf_Down","Muon_ID_Sf_Up","Muon_ID_Sf_Down","Muon_ISO_Sf_Up","Muon_ISO_Sf_Down"] #guru
  ...
  for entry in tree:
    ...
    metPt_nom = MET_pt(file, entry, "nominal")
    metPhi_nom = MET_phi(file, entry, "nominal")
    MassT = -1
    ...
    Met4vec.SetPtEtaPhiM(metPt_nom, 0, metPhi_nom, 0)
    MassT = math.sqrt(2*entry.event_Lepton_pt*metPt_nom*(1 - math.cos(Lep4vec.DeltaPhi(Met4vec))))
    ...
    evtwt_L1PreFiringWeight_Up = 1 #guru
    evtwt_L1PreFiringWeight_Dn = 1
    #guru
    evtwt_Ele_IDISO_Sf = 1
    evtwt_Ele_RECO_Sf = 1
    evtwt_Ele_Trig_Sf = 1
    evtwt_Mu_Trig_Sf = 1
    evtwt_Mu_ID_Sf = 1
    evtwt_Mu_ISO_Sf = 1
    evtwt_Ele_IDISO_Sf_Up = 1
    evtwt_Ele_IDISO_Sf_Down = 1
    evtwt_Ele_RECO_Sf_Up = 1
    evtwt_Ele_RECO_Sf_Down = 1   
    evtwt_Ele_Trig_Sf_Up = 1
    evtwt_Ele_Trig_Sf_Down = 1 
    evtwt_Mu_Trig_Sf_Up = 1
    evtwt_Mu_Trig_Sf_Down = 1
    evtwt_Mu_ID_Sf_Up = 1
    evtwt_Mu_ID_Sf_Down = 1
    evtwt_Mu_ISO_Sf_Up = 1
    evtwt_Mu_ISO_Sf_Down = 1
    ...
    if not (isMC(file)):
     ...
     evtwt_L1PreFiringWeight_Up *=evtwt #guru
     evtwt_L1PreFiringWeight_Dn *=evtwt
     #guru
     evtwt_Ele_IDISO_Sf *= evtwt
     evtwt_Ele_IDISO_Sf_Up *= evtwt
     evtwt_Ele_IDISO_Sf_Down *= evtwt
     evtwt_Ele_RECO_Sf *= evtwt
     evtwt_Ele_RECO_Sf_Up *= evtwt
     evtwt_Ele_RECO_Sf_Down *= evtwt
     evtwt_Ele_Trig_Sf  *= evtwt   
     evtwt_Ele_Trig_Sf_Up *= evtwt
     evtwt_Ele_Trig_Sf_Down *= evtwt
     evtwt_Mu_Trig_Sf *= evtwt
     evtwt_Mu_Trig_Sf_Up *= evtwt
     evtwt_Mu_Trig_Sf_Down *= evtwt
     evtwt_Mu_ID_Sf *= evtwt
     evtwt_Mu_ID_Sf_Up *= evtwt
     evtwt_Mu_ID_Sf_Down *= evtwt
     evtwt_Mu_ISO_Sf *= evtwt
     evtwt_Mu_ISO_Sf_Up *= evtwt
     evtwt_Mu_ISO_Sf_Down *= evtwt
          ...
          if (jet4vec.DeltaR(Lep4vec) > 0.5):
            ...
            if (useDeepCSV): 
                if (btagDeepCSV(entry.Jet_btagDeepB[j], "Medium") >= 1): 
                  ...
                  btagSF_evt_tree *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepcsv[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
            else: 
...
#AI: modified logic ...Also fix the systematic
#AI: https://twiki.cern.ch/twiki/bin/viewauth/CMS/BTagSFMethods#1a_Event_reweighting_using_scale                
...
#                else:
#                  btagSF_evt_tree *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
#                  if (Jet_flavour(entry, j) == 0 or Jet_flavour(entry, j) == 1):
#                    btagSF_evt_treeUp_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_up[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
#                    btagSF_evt_treeDown_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_down[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
#                  else:
#                    btagSF_evt_treeUp_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_up[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
#                    btagSF_evt_treeDown_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_down[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                if (btagDeepFLV(entry.Jet_btagDeepFlavB[j], "Tight") >= 1): 
                  btagSF_evt_tree *= entry.Jet_btagSF_deepjet_T[j]
                  if (Jet_flavour(entry, j) == 0 or Jet_flavour(entry, j) == 1):
                    btagSF_evt_treeUp_hf *= entry.Jet_btagSF_deepjet_T_up[j]
                    btagSF_evt_treeDown_hf *= entry.Jet_btagSF_deepjet_T_down[j]
                    btagSF_evt_treeUp_lf *= entry.Jet_btagSF_deepjet_T[j]
                    btagSF_evt_treeDown_lf *= entry.Jet_btagSF_deepjet_T[j]
                  else:
                    btagSF_evt_treeUp_hf *= entry.Jet_btagSF_deepjet_T[j]
                    btagSF_evt_treeDown_hf *= entry.Jet_btagSF_deepjet_T[j]
                    btagSF_evt_treeUp_lf *= entry.Jet_btagSF_deepjet_T_up[j]
                    btagSF_evt_treeDown_lf *= entry.Jet_btagSF_deepjet_T_down[j]
                elif (btagDeepFLV(entry.Jet_btagDeepFlavB[j], "Medium") >= 1) & (btagDeepFLV(entry.Jet_btagDeepFlavB[j], "Tight") < 1): 
                  #print( "pt %0.3f, eta %0.3f" %(Jet_pt(file, entry, j, "nominal"), entry.Jet_eta[j]))
                  #print( "the tight eff is %0.3f" %Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                  if (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")==0): 
                    continue
                  btagSF_evt_tree *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                  if (Jet_flavour(entry, j) == 0 or Jet_flavour(entry, j) == 1):
                    btagSF_evt_treeUp_hf *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_up[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T_up[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                    btagSF_evt_treeDown_hf *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_down[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T_down[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                    btagSF_evt_treeUp_lf *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                    btagSF_evt_treeDown_lf *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                  else:
                    btagSF_evt_treeUp_hf *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                    btagSF_evt_treeDown_hf *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                    btagSF_evt_treeUp_lf *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_up[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T_up[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                    btagSF_evt_treeDown_lf *= max(0.0, (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_down[j] - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight')*entry.Jet_btagSF_deepjet_T_down[j])) / (Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "") - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), '_Tight'))
                else: # not-tagged
                  btagSF_evt_tree *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                  if (Jet_flavour(entry, j) == 0 or Jet_flavour(entry, j) == 1):
                    btagSF_evt_treeUp_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_up[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                    btagSF_evt_treeDown_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_down[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                    btagSF_evt_treeUp_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                    btagSF_evt_treeDown_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                  else:
                    btagSF_evt_treeUp_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                    btagSF_evt_treeDown_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                    btagSF_evt_treeUp_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_up[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
                    btagSF_evt_treeDown_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), "")*entry.Jet_btagSF_deepjet_M_down[j]))/(1 - Jet_bTagEff(entry, j, Jet_pt(file, entry, j, "nominal"), abs(entry.Jet_eta[j]), ""))
      ...
      if (year  == "2016"):
          if (float(entry.event_Event_flag) == 13 and float(entry.event_MuonTrigFlag) >= 1):
            ...
            #guru
            evtwt_Mu_ID_Sf *= float(idsf)
            evtwt_Mu_ISO_Sf *= float(isosf)
            evtwt_Mu_Trig_Sf *= float(trigsf)
            
            evtwt_Mu_ID_Sf_Up *= float((0.45*float(entry.Muon_ID_GH_SFstatUp[entry.event_Lepton_idx]))+(0.55*float(entry.Muon_ID_BCDEF_SFstatUp[entry.event_Lepton_idx])))*float((0.45*float(entry.Muon_ID_GH_SFsystUp[entry.event_Lepton_idx]))+(0.55*float(entry.Muon_ID_BCDEF_SFsystUp[entry.event_Lepton_idx])))
            evtwt_Mu_ISO_Sf_Up *= np.add(np.multiply(0.45,entry.Muon_ISO_GH_SFstatUp[entry.event_Lepton_idx]), np.multiply(0.55,entry.Muon_ISO_BCDEF_SFstatUp[entry.event_Lepton_idx]))*np.add(np.multiply(0.45,entry.Muon_ISO_GH_SFsystUp[entry.event_Lepton_idx]), np.multiply(0.55,entry.Muon_ISO_BCDEF_SFsystUp[entry.event_Lepton_idx]))
            evtwt_Mu_Trig_Sf_Up *= np.add(np.multiply(0.45,entry.Muon_Trigger_GH_SFstatUp[entry.event_Lepton_idx]), np.multiply(0.55,entry.Muon_Trigger_BCDEF_SFstatUp[entry.event_Lepton_idx]))*np.add(np.multiply(0.45,entry.Muon_Trigger_GH_SFsystUp[entry.event_Lepton_idx]), np.multiply(0.55,entry.Muon_Trigger_BCDEF_SFsystUp[entry.event_Lepton_idx]))
            #guru
            evtwt_Mu_ID_Sf_Down *= float((0.45*float(entry.Muon_ID_GH_SFstatDown[entry.event_Lepton_idx]))+(0.55*float(entry.Muon_ID_BCDEF_SFstatDown[entry.event_Lepton_idx])))*float((0.45*float(entry.Muon_ID_GH_SFsystDown[entry.event_Lepton_idx]))+(0.55*float(entry.Muon_ID_BCDEF_SFsystDown[entry.event_Lepton_idx])))
            evtwt_Mu_ISO_Sf_Down *= np.add(np.multiply(0.45,entry.Muon_ISO_GH_SFstatDown[entry.event_Lepton_idx]), np.multiply(0.55,entry.Muon_ISO_BCDEF_SFstatDown[entry.event_Lepton_idx]))*np.add(np.multiply(0.45,entry.Muon_ISO_GH_SFsystDown[entry.event_Lepton_idx]), np.multiply(0.55,entry.Muon_ISO_BCDEF_SFsystDown[entry.event_Lepton_idx]))
            evtwt_Mu_Trig_Sf_Down *= np.add(np.multiply(0.45,entry.Muon_Trigger_GH_SFstatDown[entry.event_Lepton_idx]), np.multiply(0.55,entry.Muon_Trigger_BCDEF_SFstatDown[entry.event_Lepton_idx]))*np.add(np.multiply(0.45,entry.Muon_Trigger_GH_SFsystDown[entry.event_Lepton_idx]), np.multiply(0.55,entry.Muon_Trigger_BCDEF_SFsystDown[entry.event_Lepton_idx]))
            ...
            for el in range(0, entry.nElectron):
              trigsf      = float(Ele_trig_SF(entry.Electron_eta[el], entry.Electron_pt[el],""))
              evtwt  *= (float(idisosf)*float(recosf)*float(trigsf))
            #guru
            evtwt_Ele_IDISO_Sf *= float(idisosf)
            evtwt_Ele_RECO_Sf *= float(recosf)
            evtwt_Ele_Trig_Sf *= float(trigsf)
            
            evtwt_Ele_IDISO_Sf_Up     *= float(entry.Electron_IDISO_BCDEFGH_SFstatUp[entry.event_Lepton_idx])*float(entry.Electron_IDISO_BCDEFGH_SFsystUp[entry.event_Lepton_idx])
            evtwt_Ele_RECO_Sf_Up      *= float(entry.Electron_RECO_BCDEFGH_SFstatUp[entry.event_Lepton_idx])*float(entry.Electron_RECO_BCDEFGH_SFsystUp[entry.event_Lepton_idx])
            evtwt_Ele_Trig_Sf_Up      *= float(Ele_trig_SF(entry.Electron_eta[el], entry.Electron_pt[el],"Up"))
            #guru
            evtwt_Ele_IDISO_Sf_Down     *= float(entry.Electron_IDISO_BCDEFGH_SFstatDown[entry.event_Lepton_idx])*float(entry.Electron_IDISO_BCDEFGH_SFsystDown[entry.event_Lepton_idx])
            evtwt_Ele_RECO_Sf_Down      *= float(entry.Electron_RECO_BCDEFGH_SFstatDown[entry.event_Lepton_idx])*float(entry.Electron_RECO_BCDEFGH_SFsystDown[entry.event_Lepton_idx])
            evtwt_Ele_Trig_Sf_Down      *= float(Ele_trig_SF(entry.Electron_eta[el], entry.Electron_pt[el],"Down"))
          ...
          if (float(entry.event_Event_flag) == 13 and float(entry.event_MuonTrigFlag) >= 1):
            ...
            evtwt_Mu_ID_Sf_Up *= float(idsf)+quadradd(float(entry.Muon_ID_BCDEF_SFstatUp[entry.event_Lepton_idx]),float(entry.Muon_ID_BCDEF_SFsystUp[entry.event_Lepton_idx]),float(idsf))
            evtwt_Mu_ISO_Sf_Up *= float(isosf)+quadradd(float(entry.Muon_ISO_BCDEF_SFstatUp[entry.event_Lepton_idx]),float(entry.Muon_ISO_BCDEF_SFsystUp[entry.event_Lepton_idx]),float(isosf))
            evtwt_Mu_Trig_Sf_Up *= float(trigsf)+ quadradd(float(entry.Muon_Trigger_BCDEF_SFstatUp[entry.event_Lepton_idx]),float(entry.Muon_Trigger_BCDEF_SFsystUp[entry.event_Lepton_idx]),float(trigsf))
            #guru
            evtwt_Mu_ID_Sf_Down *= float(idsf)-quadradd(float(entry.Muon_ID_BCDEF_SFstatDown[entry.event_Lepton_idx]),float(entry.Muon_ID_BCDEF_SFsystDown[entry.event_Lepton_idx]),float(idsf))
            evtwt_Mu_ISO_Sf_Down *= float(isosf)-quadradd(float(entry.Muon_ISO_BCDEF_SFstatDown[entry.event_Lepton_idx]),float(entry.Muon_ISO_BCDEF_SFsystDown[entry.event_Lepton_idx]),float(isosf))
            evtwt_Mu_Trig_Sf_Down *= float(trigsf)- quadradd(float(entry.Muon_Trigger_BCDEF_SFstatDown[entry.event_Lepton_idx]),float(entry.Muon_Trigger_BCDEF_SFsystDown[entry.event_Lepton_idx]),float(trigsf))
            ...
            evtwt_Ele_IDISO_Sf_Up     *= float(idisosf)++quadradd(float(entry.Electron_IDISO_BCDEF_SFstatUp[entry.event_Lepton_idx]),float(entry.Electron_IDISO_BCDEF_SFsystUp[entry.event_Lepton_idx]),float(idisosf))
            evtwt_Ele_RECO_Sf_Up      *= float(recosf)+ quadradd(float(entry.Electron_RECO_BCDEF_SFstatUp[entry.event_Lepton_idx]),float(entry.Electron_RECO_BCDEF_SFsystUp[entry.event_Lepton_idx]),float(recosf))
            evtwt_Ele_Trig_Sf_Up      *= float(Ele_trig_SF(entry.Electron_eta[el], entry.Electron_pt[el],"Up"))
            #print("evtwt_Ele_RECO_Sf_Up: ",evtwt_Ele_RECO_Sf_Up,"statUp: ",entry.Electron_RECO_BCDEF_SFstatUp[entry.event_Lepton_idx], "sysUp: ",entry.Electron_RECO_BCDEF_SFsystUp[entry.event_Lepton_idx])
            #guru
            evtwt_Ele_IDISO_Sf_Down     *= float(idisosf)- quadradd(float(entry.Electron_IDISO_BCDEF_SFstatDown[entry.event_Lepton_idx]),float(entry.Electron_IDISO_BCDEF_SFsystDown[entry.event_Lepton_idx]),float(idisosf))
            evtwt_Ele_RECO_Sf_Down      *= float(recosf)- quadradd(float(entry.Electron_RECO_BCDEF_SFstatDown[entry.event_Lepton_idx]),float(entry.Electron_RECO_BCDEF_SFsystDown[entry.event_Lepton_idx]),float(recosf))
            evtwt_Ele_Trig_Sf_Down      *= float(Ele_trig_SF(entry.Electron_eta[el], entry.Electron_pt[el],"Down"))
          if (isTTJet(file) and applyTopPtRew): 
            #topptweight=float(entry.TopPt_Weight_Nominal)
            #topptweight= topPt_Reweighting(file, entry, 0.0615*2.11, 0.0005*0.514)
          ...
          if (float(entry.event_Event_flag) == 13 and float(entry.event_MuonTrigFlag) >= 1):
            ...
            evtwt_Mu_ID_Sf_Up *= float(entry.Muon_ID_ABCD_SFstatUp[entry.event_Lepton_idx])*float(entry.Muon_ID_ABCD_SFsystUp[entry.event_Lepton_idx])
            evtwt_Mu_ISO_Sf_Up *= float(entry.Muon_ISO_ABCD_SFstatUp[entry.event_Lepton_idx])*float(entry.Muon_ISO_ABCD_SFsystUp[entry.event_Lepton_idx])
            trigsfBefore_Up = entry.Muon_TriggerBefore_ABCD_SFstatUp[entry.event_Lepton_idx]*entry.Muon_TriggerBefore_ABCD_SFsystUp[entry.event_Lepton_idx]
            trigsfAfter_Up  = entry.Muon_TriggerAfter_ABCD_SFstatUp[entry.event_Lepton_idx]*entry.Muon_TriggerAfter_ABCD_SFsystUp[entry.event_Lepton_idx]
            evtwt_Mu_Trig_Sf_Up *= float(((trigsfBefore_Up*7.0) + (trigsfAfter_Up*53.0))/60.0)
            #guru
            evtwt_Mu_ID_Sf_Down *= float(entry.Muon_ID_ABCD_SFstatDown[entry.event_Lepton_idx])*float(entry.Muon_ID_ABCD_SFsystDown[entry.event_Lepton_idx])
            evtwt_Mu_ISO_Sf_Down *= float(entry.Muon_ISO_ABCD_SFstatDown[entry.event_Lepton_idx])*float(entry.Muon_ISO_ABCD_SFsystDown[entry.event_Lepton_idx])
            trigsfBefore_Down = entry.Muon_TriggerBefore_ABCD_SFstatDown[entry.event_Lepton_idx]*entry.Muon_TriggerBefore_ABCD_SFsystDown[entry.event_Lepton_idx]
            trigsfAfter_Down  = entry.Muon_TriggerAfter_ABCD_SFstatDown[entry.event_Lepton_idx]*entry.Muon_TriggerAfter_ABCD_SFsystDown[entry.event_Lepton_idx]
            evtwt_Mu_Trig_Sf_Down *= float(((trigsfBefore_Down*7.0) + (trigsfAfter_Down*53.0))/60.0)
            ...
            evtwt_Ele_IDISO_Sf_Up     *= float(entry.Electron_IDISO_ABCD_SFstatUp[entry.event_Lepton_idx])*float(entry.Electron_IDISO_ABCD_SFsystUp[entry.event_Lepton_idx])
            evtwt_Ele_RECO_Sf_Up      *= float(entry.Electron_RECO_ABCD_SFstatUp[entry.event_Lepton_idx])*float(entry.Electron_RECO_ABCD_SFsystUp[entry.event_Lepton_idx])
            evtwt_Ele_Trig_Sf_Up      *= float(Ele_trig_SF(entry.Electron_eta[el], entry.Electron_pt[el],"Up"))
            #guru
            evtwt_Ele_IDISO_Sf_Down     *= float(entry.Electron_IDISO_ABCD_SFstatDown[entry.event_Lepton_idx])*float(entry.Electron_IDISO_ABCD_SFsystDown[entry.event_Lepton_idx])
            evtwt_Ele_RECO_Sf_Down      *= float(entry.Electron_RECO_ABCD_SFstatDown[entry.event_Lepton_idx])*float(entry.Electron_RECO_ABCD_SFsystDown[entry.event_Lepton_idx])
            evtwt_Ele_Trig_Sf_Down      *= float(Ele_trig_SF(entry.Electron_eta[el], entry.Electron_pt[el],"Down"))
      ...
      if (year=="2016" or year=="2017"):
        L1prefiringwt_nom = float(entry.L1PreFiringWeight_Nom)
        L1prefiringwt_up = float(entry.L1PreFiringWeight_Up)
        L1prefiringwt_dn = float(entry.L1PreFiringWeight_Dn)
        
      evtwt_nominal = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
      evtwt_nominal_nopu = evtwt*float(entry.genWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
      evtwt_nominalv2 = evtwt*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
      if (doSys):
        evtwt_PileupUp = evtwt*float(entry.genWeight)*float(entry.puWeightUp)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
        evtwt_PileupDown = evtwt*float(entry.genWeight)*float(entry.puWeightDown)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
        evtwt_BTaghfSFUp = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_treeUp_hf)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
        evtwt_BTaghfSFDown = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_treeDown_hf)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
        evtwt_BTaglfSFUp = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_treeUp_lf)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
        evtwt_BTaglfSFDown = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_treeDown_lf)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
        evtwt_topptweightUp = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweightUp)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
        evtwt_topptweightDown = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweightDown)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*L1prefiringwt_nom
        evtwt_TTJets_ST_ScalingUp = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*TTJets_ST_ScalingUp*L1prefiringwt_nom
        evtwt_TTJets_ST_ScalingDown = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*TTJets_ST_ScalingDown*L1prefiringwt_nom
        ...
        #guru
        evtwt_L1PreFiringWeight_Up = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*L1prefiringwt_up
        evtwt_L1PreFiringWeight_Dn = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*L1prefiringwt_dn
        evtwt_Ele_IDISO_Sf_Up = evtwt_nominal*evtwt_Ele_IDISO_Sf_Up/evtwt_Ele_IDISO_Sf
        evtwt_Ele_IDISO_Sf_Down = evtwt_nominal*evtwt_Ele_IDISO_Sf_Down/evtwt_Ele_IDISO_Sf
        evtwt_Ele_RECO_Sf_Up = evtwt_nominal*evtwt_Ele_RECO_Sf_Up/evtwt_Ele_RECO_Sf
        evtwt_Ele_RECO_Sf_Down = evtwt_nominal*evtwt_Ele_RECO_Sf_Down/evtwt_Ele_RECO_Sf
        evtwt_Ele_Trig_Sf_Up = evtwt_nominal*evtwt_Ele_Trig_Sf_Up/evtwt_Ele_Trig_Sf
        evtwt_Ele_Trig_Sf_Down = evtwt_nominal*evtwt_Ele_Trig_Sf_Down/evtwt_Ele_Trig_Sf
        evtwt_Mu_Trig_Sf_Up = evtwt_nominal*evtwt_Mu_Trig_Sf_Up/evtwt_Mu_Trig_Sf
        evtwt_Mu_Trig_Sf_Down = evtwt_nominal*evtwt_Mu_Trig_Sf_Down/evtwt_Mu_Trig_Sf
        evtwt_Mu_ID_Sf_Up = evtwt_nominal*evtwt_Mu_ID_Sf_Up/evtwt_Mu_ID_Sf
        evtwt_Mu_ID_Sf_Down = evtwt_nominal*evtwt_Mu_ID_Sf_Down/evtwt_Mu_ID_Sf
        evtwt_Mu_ISO_Sf_Up = evtwt_nominal*evtwt_Mu_ISO_Sf_Up/evtwt_Mu_ISO_Sf
        evtwt_Mu_ISO_Sf_Down = evtwt_nominal*evtwt_Mu_ISO_Sf_Down/evtwt_Mu_ISO_Sf
    ...
    if (isMC(file)):
      if (MET_Filters(file, entry) >= 1 and entry.event_Lepton_pt > 40 and abs(entry.event_Lepton_eta) < 2.1 and entry.event_LeadJet_pt > 200 and abs(entry.event_LeadJet_eta) < 2.4 and entry.event_DR_LepleadJet > 0.5 and metPt_nom > 60 and entry.event_ST_v2 > 500 and  HT_Central > 500 and MassT < 130):
...
# AI: Fill Tight b-tag efficiency Map          
              if (Jet_pt(file, entry, j, "nominal") > 30 and abs(entry.Jet_eta[j]) < 2.4 and JetID(entry.Jet_jetId[j]) >= 1 and btagDeepFLV(entry.Jet_btagDeepFlavB[j], "Tight") >= 1):
                if (Jet_flavour(entry, j) == 0): effmap_pt_eta_b_btagged_Tight.Fill(Jet_pt(file, entry, j, "nominal"), entry.Jet_eta[j]) 
                elif (Jet_flavour(entry, j) == 1): effmap_pt_eta_c_btagged_Tight.Fill(Jet_pt(file, entry, j, "nominal"), entry.Jet_eta[j]) 
                elif (Jet_flavour(entry, j) == 2): effmap_pt_eta_l_btagged_Tight.Fill(Jet_pt(file, entry, j, "nominal"), entry.Jet_eta[j])    
    ...
    for cr in range(len(controlR)):
      bdtOutput, cut = cutFlow(file, entry, str(controlR[cr]), leadjetBTag, HT_Central, nBTag, MassT, LepCJ_pt, LepCJ_eta, LepCJ_btag, M_Top, Mjj_lJ_CJ, DR_lJ_CJ, HT_Full_pu_var, reader, metPt_nom, float(entry.event_LeadJet_pt), dR_lepClosestJet_v2, nVeto)
      for itr in range (0, int(cut)):
          ...
          if (itr == 16):
            fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "nominal", evtwt_nominal)
            if (doSys):
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "NoPileup", evtwt_nominal_nopu)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,"topptweightUp", evtwt_topptweightUp)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,"topptweightDown", evtwt_topptweightDown)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt, "BTaghfSFUp", evtwt_BTaghfSFUp)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt, "BTaghfSFDown", evtwt_BTaghfSFDown)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt, "BTaglfSFUp", evtwt_BTaglfSFUp)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt, "BTaglfSFDown", evtwt_BTaglfSFDown)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "PileupUp", evtwt_PileupUp)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "PileupDown", evtwt_PileupDown)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEScale1", evtwt_lhescale1)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEScale2", evtwt_lhescale2)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEScale3", evtwt_lhescale3)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEScale4", evtwt_lhescale4)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEScale6", evtwt_lhescale6)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEScale8", evtwt_lhescale8)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEScaleDownWeight", evtwt_LHEScaleDownWeight)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEScaleUpWeight", evtwt_LHEScaleUpWeight)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEPdfDownWeight", evtwt_LHEPdfDownWeight)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "LHEPdfUpWeight", evtwt_LHEPdfUpWeight)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "TTJets_ST_ScalingUp", evtwt_TTJets_ST_ScalingUp)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "TTJets_ST_ScalingDown", evtwt_TTJets_ST_ScalingDown)
              #guru
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "L1PreFiringWeight_Up", evtwt_L1PreFiringWeight_Up)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "L1PreFiringWeight_Dn", evtwt_L1PreFiringWeight_Dn)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Electron_IDISO_Sf_Up", evtwt_Ele_IDISO_Sf_Up)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Electron_IDISO_Sf_Down", evtwt_Ele_IDISO_Sf_Down)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Electron_RECO_Sf_Up", evtwt_Ele_RECO_Sf_Up)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Electron_RECO_Sf_Down", evtwt_Ele_RECO_Sf_Down)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Electron_Trig_Sf_Up", evtwt_Ele_Trig_Sf_Up)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Electron_Trig_Sf_Down", evtwt_Ele_Trig_Sf_Down)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Muon_Trig_Sf_Up", evtwt_Mu_Trig_Sf_Up)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Muon_Trig_Sf_Down", evtwt_Mu_Trig_Sf_Down)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Muon_ID_Sf_Up", evtwt_Mu_ID_Sf_Up)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Muon_ID_Sf_Down", evtwt_Mu_ID_Sf_Down)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Muon_ISO_Sf_Up", evtwt_Mu_ISO_Sf_Up)
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, metPt_nom, metPhi_nom, entry.event_LeadJet_pt,  "Muon_ISO_Sf_Down",  evtwt_Mu_ISO_Sf_Down)

          if (itr == 4 and doSkim and "PreSig" in str(controlR[cr])):
            #Nominal 
            loadSkimTree(file, entry, event_genwt, leadjetBTag, MassT, HT_Central, HT_Full_pu_var, HT_Full_nopu_var, neles, nmuons, evtwt_nominal, evtwt_PileupUp, evtwt_PileupDown, evtwt_BTaghfSFUp, evtwt_BTaghfSFDown, evtwt_BTaglfSFUp, evtwt_BTaglfSFDown, evtwt_LHEScaleUpWeight, evtwt_LHEScaleDownWeight, evtwt_LHEPdfUpWeight, evtwt_LHEPdfDownWeight, evtwt_TTJets_ST_ScalingUp, evtwt_TTJets_ST_ScalingDown, lepClosestJet_v2_4vec, Lep4vec, Met4vec, leadJet_4vec, leadjetBTagTig, FwdJetPt_T, fwdJeteta_T, Mt_js, Mw_js, metPt_nom, entry.event_LeadJet_pt,evtwt_Ele_IDISO_Sf_Up, evtwt_Ele_IDISO_Sf_Down, evtwt_Ele_RECO_Sf_Up, evtwt_Ele_RECO_Sf_Down, evtwt_Ele_Trig_Sf_Up, evtwt_Ele_Trig_Sf_Down, evtwt_Mu_Trig_Sf_Up, evtwt_Mu_Trig_Sf_Down, evtwt_Mu_ID_Sf_Up, evtwt_Mu_ID_Sf_Down, evtwt_Mu_ISO_Sf_Up,
evtwt_Mu_ISO_Sf_Down)
            sktree.Fill()
      ...
      for cr in range(len(controlR)):
        ...
        if ('Mu' in CHL or 'Ele' in CHL):
          ...
          if (int(cutjerUp) >= 5 and doSkim and "PreSig" in str(controlR[cr])):
            loadSkimTree(file, entry, event_genwt, leadjetBTag, MassT, HT_Central, HT_Full_pu_var, HT_Full_nopu_var, neles, nmuons, evtwt_nominal, evtwt_PileupUp, evtwt_PileupDown, evtwt_BTaghfSFUp, evtwt_BTaghfSFDown, evtwt_BTaglfSFUp, evtwt_BTaglfSFDown, evtwt_LHEScaleUpWeight, evtwt_LHEScaleDownWeight, evtwt_LHEPdfUpWeight, evtwt_LHEPdfDownWeight, evtwt_TTJets_ST_ScalingUp, evtwt_TTJets_ST_ScalingDown, lepClosestJet_v2_4vec, Lep4vec, Met4vec, leadJet_4vec, leadjetBTagTig, FwdJetPt_T, fwdJeteta_T, Mt_js, Mw_js, MET_pt(file, entry, "jerUp"), Jet_pt(file, entry, entry.event_LeadJet_idx, "jerUp"),evtwt_Ele_IDISO_Sf_Up, evtwt_Ele_IDISO_Sf_Down, evtwt_Ele_RECO_Sf_Up, evtwt_Ele_RECO_Sf_Down, evtwt_Ele_Trig_Sf_Up, evtwt_Ele_Trig_Sf_Down, evtwt_Mu_Trig_Sf_Up, evtwt_Mu_Trig_Sf_Down, evtwt_Mu_ID_Sf_Up, evtwt_Mu_ID_Sf_Down, evtwt_Mu_ISO_Sf_Up,
evtwt_Mu_ISO_Sf_Down)
            sktree_jerUp.Fill()
          if (int(cutjerDown) >= 5 and doSkim and "PreSig" in str(controlR[cr])):
            loadSkimTree(file, entry, event_genwt, leadjetBTag, MassT, HT_Central, HT_Full_pu_var, HT_Full_nopu_var, neles, nmuons, evtwt_nominal, evtwt_PileupUp, evtwt_PileupDown, evtwt_BTaghfSFUp, evtwt_BTaghfSFDown, evtwt_BTaglfSFUp, evtwt_BTaglfSFDown, evtwt_LHEScaleUpWeight, evtwt_LHEScaleDownWeight, evtwt_LHEPdfUpWeight, evtwt_LHEPdfDownWeight, evtwt_TTJets_ST_ScalingUp, evtwt_TTJets_ST_ScalingDown, lepClosestJet_v2_4vec, Lep4vec, Met4vec, leadJet_4vec, leadjetBTagTig, FwdJetPt_T, fwdJeteta_T, Mt_js, Mw_js, MET_pt(file, entry, "jerDown"), Jet_pt(file, entry, entry.event_LeadJet_idx, "jerDown"),evtwt_Ele_IDISO_Sf_Up, evtwt_Ele_IDISO_Sf_Down, evtwt_Ele_RECO_Sf_Up, evtwt_Ele_RECO_Sf_Down, evtwt_Ele_Trig_Sf_Up, evtwt_Ele_Trig_Sf_Down, evtwt_Mu_Trig_Sf_Up, evtwt_Mu_Trig_Sf_Down, evtwt_Mu_ID_Sf_Up, evtwt_Mu_ID_Sf_Down, evtwt_Mu_ISO_Sf_Up,
evtwt_Mu_ISO_Sf_Down)
            sktree_jerDown.Fill()
          if (int(cutjesUp) >= 5 and doSkim and "PreSig" in str(controlR[cr])):
            loadSkimTree(file, entry, event_genwt, leadjetBTag, MassT, HT_Central, HT_Full_pu_var, HT_Full_nopu_var, neles, nmuons, evtwt_nominal, evtwt_PileupUp, evtwt_PileupDown, evtwt_BTaghfSFUp, evtwt_BTaghfSFDown, evtwt_BTaglfSFUp, evtwt_BTaglfSFDown, evtwt_LHEScaleUpWeight, evtwt_LHEScaleDownWeight, evtwt_LHEPdfUpWeight, evtwt_LHEPdfDownWeight, evtwt_TTJets_ST_ScalingUp, evtwt_TTJets_ST_ScalingDown, lepClosestJet_v2_4vec, Lep4vec, Met4vec, leadJet_4vec, leadjetBTagTig, FwdJetPt_T, fwdJeteta_T, Mt_js, Mw_js, MET_pt(file, entry, "jesUp"), Jet_pt(file, entry, entry.event_LeadJet_idx, "jesUp"),evtwt_Ele_IDISO_Sf_Up, evtwt_Ele_IDISO_Sf_Down, evtwt_Ele_RECO_Sf_Up, evtwt_Ele_RECO_Sf_Down, evtwt_Ele_Trig_Sf_Up, evtwt_Ele_Trig_Sf_Down, evtwt_Mu_Trig_Sf_Up, evtwt_Mu_Trig_Sf_Down, evtwt_Mu_ID_Sf_Up, evtwt_Mu_ID_Sf_Down, evtwt_Mu_ISO_Sf_Up,
evtwt_Mu_ISO_Sf_Down)
            sktree_jesUp.Fill()
          if (int(cutjesDown) >= 5 and doSkim and "PreSig" in str(controlR[cr])):
            loadSkimTree(file, entry, event_genwt, leadjetBTag, MassT, HT_Central, HT_Full_pu_var, HT_Full_nopu_var, neles, nmuons, evtwt_nominal, evtwt_PileupUp, evtwt_PileupDown, evtwt_BTaghfSFUp, evtwt_BTaghfSFDown, evtwt_BTaglfSFUp, evtwt_BTaglfSFDown, evtwt_LHEScaleUpWeight, evtwt_LHEScaleDownWeight, evtwt_LHEPdfUpWeight, evtwt_LHEPdfDownWeight, evtwt_TTJets_ST_ScalingUp, evtwt_TTJets_ST_ScalingDown, lepClosestJet_v2_4vec, Lep4vec, Met4vec, leadJet_4vec, leadjetBTagTig, FwdJetPt_T, fwdJeteta_T, Mt_js, Mw_js, MET_pt(file, entry, "jesDown"), Jet_pt(file, entry, entry.event_LeadJet_idx, "jesDown"),evtwt_Ele_IDISO_Sf_Up, evtwt_Ele_IDISO_Sf_Down, evtwt_Ele_RECO_Sf_Up, evtwt_Ele_RECO_Sf_Down, evtwt_Ele_Trig_Sf_Up, evtwt_Ele_Trig_Sf_Down, evtwt_Mu_Trig_Sf_Up, evtwt_Mu_Trig_Sf_Down, evtwt_Mu_ID_Sf_Up, evtwt_Mu_ID_Sf_Down, evtwt_Mu_ISO_Sf_Up,
evtwt_Mu_ISO_Sf_Down)
            sktree_jesDown.Fill()
