...
for i in  range(1):
  ...
  for entry in tree:
    ...
    if (isMC(file)):
      ...
      evtwt_nominal = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
      evtwt_nominal_nopu = evtwt*float(entry.genWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
      evtwt_nominalv2 = evtwt*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
      if (doSys):
        evtwt_PileupUp = evtwt*float(entry.genWeight)*float(entry.puWeightUp)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
        evtwt_PileupDown = evtwt*float(entry.genWeight)*float(entry.puWeightDown)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
        evtwt_BTaghfSFUp = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_treeUp_hf)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
        evtwt_BTaghfSFDown = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_treeDown_hf)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
        evtwt_BTaglfSFUp = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_treeUp_lf)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
        evtwt_BTaglfSFDown = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_treeDown_lf)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
        evtwt_topptweightUp = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweightUp)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
        evtwt_topptweightDown = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweightDown)*float(wpt_WJets_Scaling(file, entry))*st_TTJets_Scaling(file, entry)*float(entry.L1PreFiringWeight_Nom)
        ...
        evtwt_L1PreFiringWeight_Up = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*float(entry.L1PreFiringWeight_Up)
        evtwt_L1PreFiringWeight_Dn = evtwt*float(entry.genWeight)*float(entry.puWeight)*float(btagSF_evt_tree)*float(topptweight)*float(wpt_WJets_Scaling(file, entry))*float(entry.L1PreFiringWeight_Dn)
