...
def Jet_bTagEff(entry, idx, jetpt, jeteta, cut):
  ...
  if (abs (entry.Jet_partonFlavour[idx]) ==  5): 
     eff_hist  = eff_file.Get("eff_b"+cut)
     eff = eff_hist.GetBinContent(eff_hist.GetXaxis().FindBin(jetpt), eff_hist.GetYaxis().FindBin(jeteta))
  elif (abs (entry.Jet_partonFlavour[idx]) ==  4):
     eff_hist  = eff_file.Get("eff_c"+cut)
     eff = eff_hist.GetBinContent(eff_hist.GetXaxis().FindBin(jetpt), eff_hist.GetYaxis().FindBin(jeteta))
  else : 
     eff_hist  = eff_file.Get("eff_l"+cut)
     eff = eff_hist.GetBinContent(eff_hist.GetXaxis().FindBin(jetpt), eff_hist.GetYaxis().FindBin(jeteta))
  return eff
