...
#AI: Need to read these efficiencies from the histogram
def Jet_bTagEff(entry, idx):
    ...
    return eff
#AI: Need to add the Tight b-tag histogram and read the efficiencies from the histogram
#AI: dummy values for now..
def Jet_bTagEff_Tight(entry, idx):
    eff = 0
    if (abs (entry.Jet_partonFlavour[idx]) ==  5): eff  = 0.45
    elif (abs (entry.Jet_partonFlavour[idx]) ==  4): eff  = 0.10
    else : eff  = 0.003
    return eff
...
for i in  range(1):
  ...
  systematicS = ["nominal"]
  if (doSys):
    systematicS = ["nominal", "PileupUp", "PileupDown", "BTaghfSFUp", "BTaghfSFDown", "BTaglfSFUp", "BTaglfSFDown", "topptweightUp", "topptweightDown", "jerUp", "jerDown", "jesUp", "jesDown", "LHEScale1", "LHEScale2", "LHEScale3", "LHEScale4", "LHEScale6", "LHEScale8", "LHEScaleUpWeight", "LHEScaleDownWeight", "LHEPdfUpWeight", "LHEPdfDownWeight","TTJets_ST_ScalingUp", "TTJets_ST_ScalingDown", "NoPileup","reFiringWeight_Up","L1PreFiringWeight_Dn"] #guru
...
#                    btagSF_evt_treeDown_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_down[j]))/(1 - Jet_bTagEff(entry, j))
                if (btagDeepFLV(entry.Jet_btagDeepFlavB[j], "Tight") >= 1): 
                  ...
                  btagSF_evt_tree *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                  if (Jet_flavour(entry, j) == 0 or Jet_flavour(entry, j) == 1):
                    btagSF_evt_treeUp_hf *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_up[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T_up[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                    btagSF_evt_treeDown_hf *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_down[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T_down[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                    btagSF_evt_treeUp_lf *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                    btagSF_evt_treeDown_lf *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                  else:
                    btagSF_evt_treeUp_hf *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                    btagSF_evt_treeDown_hf *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                    btagSF_evt_treeUp_lf *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_up[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T_up[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                    btagSF_evt_treeDown_lf *= max(0.0, (Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_down[j] - Jet_bTagEff_Tight(entry, j)*entry.Jet_btagSF_deepjet_T_down[j])) / (Jet_bTagEff(entry, j) - Jet_bTagEff_Tight(entry, j))
                else: # not-tagged
                  btagSF_evt_tree *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j))
                  if (Jet_flavour(entry, j) == 0 or Jet_flavour(entry, j) == 1):
                    btagSF_evt_treeUp_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_up[j]))/(1 - Jet_bTagEff(entry, j))
                    btagSF_evt_treeDown_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_down[j]))/(1 - Jet_bTagEff(entry, j))
                    btagSF_evt_treeUp_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j))
                    btagSF_evt_treeDown_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j))
                  else:
                    btagSF_evt_treeUp_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j))
                    btagSF_evt_treeDown_hf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M[j]))/(1 - Jet_bTagEff(entry, j))
                    btagSF_evt_treeUp_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_up[j]))/(1 - Jet_bTagEff(entry, j))
                    btagSF_evt_treeDown_lf *= (1 - min(1.0 , Jet_bTagEff(entry, j)*entry.Jet_btagSF_deepjet_M_down[j]))/(1 - Jet_bTagEff(entry, j))
      ...
      for itr in range (0, int(cut)):
          ...
          if (itr == 16):
            ...
            if (doSys):
              ...
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, MET_pt(file, entry, "nominal"), MET_phi(file, entry, "nominal"), entry.event_LeadJet_pt,  "L1PreFiringWeight_Up", evtwt_L1PreFiringWeight_Up)#guru
              fillHisto(str(controlR[cr]), str(ch), file, entry, itr, nBTag, MassT, Mt_js, HT_Central, leadjetBTagTig, bdtOutput, HEMwt, nCentralJets_v2, dR_lepClosestJet_v2, FwdJetPt_L, FwdJetPt_M, FwdJetPt_T, MET_pt(file, entry, "nominal"), MET_phi(file, entry, "nominal"), entry.event_LeadJet_pt,  "L1PreFiringWeight_Dn", evtwt_L1PreFiringWeight_Dn)
