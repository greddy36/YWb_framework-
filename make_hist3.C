 ...
 while ((key = (TKey*)nextkey())){
  ...
  /* if (s.find("signal") < s.length()){//clone signal histograms
	...
	/*	 if (s.find("plus") < s.length() || s.find("minus") < s.length())
	 ...
	 else if (s.find("stop") < s.length()){	 
		 TF1 *g2 = new TF1("m2", "gaus", 1300, cut);
		 TF1 *g1 = new TF1("m1", "landau", xmin, 1300);	 
		 TF1 *fitFn = new TF1("fitFn", "landau(0)*gaus(3)", xmin, cut);//for stop
	 ...
	 else if (s.find("_top") < s.length()){	 
		 TF1 *g2 = new TF1("m2", "gaus", 1300, cut);
		 TF1 *g1 = new TF1("m1", "landau", xmin, 1300);
	 	 TF1 *fitFn = new TF1("fitFn", "landau(0)-gaus(3)", xmin, cut);//for top
