...
double lognormal_pdf(double *var, double *par){ //S = S0+S1* ln(x) +S2*x+S3*x^2 
  ...
  if ((x-x0) <= 0)
    ...
    return 1129/ ((x-x0) * std::fabs(sigma) * std::sqrt(2 * M_PI)) * std::exp(-(tmp * tmp) /2);//40000-top, 10000-wjet, 42000-stop, 1129-zjets
  } 
}
...
void test(){
 ...
 TH1F *hpx = (TH1F*)f->Get("ljets__qcd");
 asymgausexp->SetParameters(hpx->GetRMS(),0, 0);
 hpx->Fit("asymgausexp");
