...
void new_lognormal_3chan(){//2gaus+expo tail
 ...
 int bin_scale = 2;   
 if(bin_scale == 2){ 
	ofile = new TFile("nuisance_hist_3chan_HC_wqcdy.root", "RECREATE");
 }
 else if(bin_scale == 4){ 
	ofile = new TFile("nuisance_hist_3chan_noFwdStop_100bins_wqcdy.root", "RECREATE");
 }
 else{

	ofile = new TFile("nuisance_hist_3chan_noFwdStop_25bins_wqcdy.root", "RECREATE");
 }
 ofstream tablefile;
