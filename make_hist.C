...
void make_hist(){//lognormal
 TFile *f = new TFile("templates_All.root");
 TH1F *hpx = (TH1F*)f->Get("All__stop__scalestop__plus"); 
 int xmin = hpx->GetXaxis()->GetXmin(); 
 int xmax = hpx->GetXaxis()->GetXmax();
 ...
 fitFn_plus->SetParameter(0,fitFn->GetParameter(0) + fitFn->GetParError(0));
 fitFn_plus->SetParameter(1,fitFn->GetParameter(1) + fitFn->GetParError(1));
 fitFn_plus->SetParameter(2,fitFn->GetParameter(2) + fitFn->GetParError(2));
 fitFn_plus->SetParameter(3,fitFn->GetParameter(3) + fitFn->GetParError(3));
 fitFn_plus->SetParameter(4,fitFn->GetParameter(4) + fitFn->GetParError(4));
 fitFn_plus->Draw("same");
