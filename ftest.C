...
//-----------------------------------------------------
	 TF1 *fitFn = new TF1("fit_new", "test_new", xmin+1*binw, xmax-binw, param_num);		  
/*		//finding stddev and mean for log(x) histogram
	 ...
	 /*if(s.find("top__b1M") < s.length()){//50bin
	 	 fitFn->SetParameters(7.5, 1100, 187, 0.29, .67,2.4);
	 }
	 if(s.find("wjets__b1M") < s.length()){
	 	 fitFn->SetParameters(43, 1200, 220, 0.3, -8,0.117);
	 }
	 if(s.find("top__b1T") < s.length()){
	 	 fitFn->SetParameters(5, 1200, -219, 0.4, 1.7,0.45);
	 }
	 if(s.find("wjets__b1T") < s.length()){
	 	 fitFn->SetParameters(11.5, 1300, 230, 0.26, -1.2,-0.2);
	 }
	 if(s.find("top__b2M") < s.length()){
	 	 fitFn->SetParameters(6, 1030, 164, 0.34, 13,4); 
	 }
	 if(s.find("wjets__b2M") < s.length()){
	 	 fitFn->SetParameters(6.5, 1100, 170, 1.3, 1.5,0.15);
	 }	 
	 if(s.find("stop__b1M") < s.length()){
	 	 fitFn->SetParameters(5, 1200, 170, 0.35, 1.6,1.43);
	 }
	 if(s.find("stop__b1T") < s.length()){
	 	 fitFn->SetParameters(5.5, 1300, 240, 0.25, 2.3,2);
	 }
	 if(s.find("stop__b2M") < s.length()){
	 	 fitFn->SetParameters(19, 1100, 170, 0.6, 1.3,0.04);
	 }*/
   ...
   fit_hist->SetBinContent(1,hpx->GetBinContent(1)); //fit_hist->SetBinContent(2,hpx->GetBinContent(1));
