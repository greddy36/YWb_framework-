...
double test_new(double *var, double *par){  
  double x = var[0], Ngaus = par[0], mean = par[1], sigma = par[2], sigma1 = par[3],slope= par[4], n = par[5],a=par[6];//, a=par[6];//,slope2=par[7], n2=par[8],b=par[9];
  if (x <= mean){
    double tmp = (x-mean)/sigma;
    return Ngaus*exp(-tmp*tmp/2);
  }
    ...
    double tmp = std::log(x/mean)/sigma1;// double tmp2= log((1-a*x-b*x*x)/(1-a*mean-b*mean*mean)); 
    return Ngaus*exp(-exp(slope)*pow(tmp*sigma1,1+exp(n))-tmp*tmp/2-tmp*tmp*tmp*a);// -tmp*tmp*tmp*a);//- exp(slope2)*pow(tmp2,1+exp(n2)));//log((1-a*x-b*x*x)/(1-a*mean-b*mean*mean))); 
    //return Ngaus*exp(-tmp*tmp/2);//-tmp/sigma3);
  }
}
...
void ftest1(){//2gaus+expo tail
 ...
 ofstream tablefile;
 tablefile.open ("table_ftest1.txt");
...
/*		//finding stddev and mean for log(x) histogram
	 ...
	 fitFn->SetParNames("Ngaus","Mean","sigma","sigma2","slope","n","A");
	 if(s.find("top__b1M") < s.length()){
	 	 fitFn->SetParameters(15, 1100, 187, 0.29, .67,2.4,1);
	 }
	 if(s.find("wjets__b1M") < s.length()){
	 	 fitFn->SetParameters(86, 1200, 220, 0.3, -8,0.117,1);
	 }
	 if(s.find("top__b1T") < s.length()){
	 	 fitFn->SetParameters(10, 1200, -219, 0.4, 1.7,0.45,1);
	 }
	 if(s.find("wjets__b1T") < s.length()){
	 	 fitFn->SetParameters(21, 1300, 230, 0.26, -1.2,-0.2,1);
	 }
	 if(s.find("top__b2M") < s.length()){
	 	 fitFn->SetParameters(10, 1100, 170, 0.35, 1.4,2.5,1);
	 }
	 if(s.find("wjets__b2M") < s.length()){
	 	 fitFn->SetParameters(13, 1100, 170, 1.3, 1.5,0.15,1);
	 }	 
	 if(s.find("stop__b1M") < s.length()){
	 	 fitFn->SetParameters(10, 1200, 170, 0.35, 1.6,1.43,-0.05);
	 }
	 if(s.find("stop__b1T") < s.length()){
	 	 fitFn->SetParameters(13, 1300, 240, 0.25, 2.3,2,-0.05);
	 }
	 if(s.find("stop__b2M") < s.length()){
	 	 fitFn->SetParameters(50, 1100, 170, 0.3, 12,5,-0.05);
	 }
	 /*if(s.find("top__b1M") < s.length()){
	 	 fitFn->SetParameters(15, 1100, 187, 0.29, .67,2.4,1);
	 }
